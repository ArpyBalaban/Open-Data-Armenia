...
def aggregate_country_products_value(
    ...
    grouped["total_value_m"] = grouped["total_value"] / 1000
    return grouped
...
all_periods_raw = sorted(set(exports_df["period_label"].dropna().unique()) | set(imports_df["period_label"].dropna().unique()))
...
with tab1:
    top_exp = exports_c.head(top_n).sort_values("total_value_m", ascending=True)
    top_imp = imports_c.head(top_n).sort_values("total_value_m", ascending=True)
    ...
    exp_chart = px.bar(
        ...
        labels={"total_value_m": {"en": "USD (millions)", "hy": "ԱՄՆ դոլար (միլիոն)"}[lang], country_label_col: {"en": "Country", "hy": "Երկիր"}[lang]},
    ...
    exp_chart.update_traces(hovertemplate="%{y}<br>Value: $%{x:,.1f}M<extra></extra>")
    ...
    imp_chart.update_traces(hovertemplate="%{y}<br>Value: $%{x:,.1f}M<extra></extra>")
...
with tab2:
    ...
    if selected_country_am:
        ...
        a, b, c = st.columns(3)
        a.metric(t("exports"), format_usd_m(exp_val))
        b.metric(t("imports"), format_usd_m(imp_val))
        c.metric(t("total_trade"), format_usd_m(total_trade))
        ...
        exp_prod_chart = px.bar(
            exp_prod.sort_values("total_value_m", ascending=True),
            x="total_value_m",
            y="display_label_short",
            orientation="h",
            title={"en": f"Top {top_n} Export Products", "hy": f"Թոփ {top_n} արտահանվող ապրանքներ"}[lang],
            labels={"total_value_m": {"en": "USD (millions)", "hy": "ԱՄՆ դոլար (միլիոն)"}[lang], "display_label_short": {"en": "Product", "hy": "Ապրանք"}[lang]},
            color_discrete_sequence=[EXPORT_COLOR],
        )
        ...
        exp_prod_chart.update_traces(
            ...
            hovertemplate="<b>%{customdata[1]}</b><br>Code: %{customdata[0]}<br>Value: $%{x:,.1f}M<extra></extra>",
        )

        imp_prod_chart = px.bar(
            imp_prod.sort_values("total_value_m", ascending=True),
            x="total_value_m",
            y="display_label_short",
            orientation="h",
...
with tab3:
    ...
    if flow == t("exports"):
        ...
        value_label = {"en": "Export Value (USD, millions)", "hy": "Արտահանման արժեք (ԱՄՆ դոլար, միլիոն)"}[lang]
        color_scale = "Blues"
    else:
        map_df = imports_c.copy()
        value_label = {"en": "Import Value (USD, millions)", "hy": "Ներմուծման արժեք (ԱՄՆ դոլար, միլիոն)"}[lang]
        color_scale = "Reds"
    ...
    if mappable.empty:
        ...
        choropleth = px.choropleth(
            ...
            labels={"total_value_m": value_label},
        )
        choropleth.update_traces(
            hovertemplate="<b>%{hovertext}</b><br>" + "Value: $%{z:,.1f}M<extra></extra>"
        )
