...
PRODUCT_LOOKUP_PATH = BASE_DIR / "data" / "clean" / "imports_products.csv"
OUTPUT_PATH = BASE_DIR / "data" / "clean" / "imports_country_products.csv"
...
ERKAPR_FILE_PATTERN = re.compile(
    r"^vt_artar_imp_erkapr_(?P<year_from>\d{4})_(?P<year_to>\d{4})(?P<suffix>_Ikis)?\.(?P<ext>xls|xlsx)$",
    re.IGNORECASE,
)
...
def load_product_name_lookup() -> dict[str, str]:
    if not PRODUCT_LOOKUP_PATH.exists():
        raise SystemExit(
            ...
            "Run scripts/parse_imports_products.py first."
        )
        ...
        rows.append(
            {
                ...
                # we enrich from the clean imports product parser output.
